								...
								<div class="card-body">
									...
									<div class="row">
										<div class="col-12 blog-details">
											<form action="{{ route('brand-message.update',$brand_message->id) }}" method="post">
												@method('PUT')
                								@csrf
												...
												<div class="form-group">
					                                ...
					                                <input type="date" class="form-control" id="end_date" name="end_date" value="{{$brand_message->end_date}}">
												...
												<div class="form-group">
													...
													<select name="city_id[]" class="select2-multiple_ form-control" multiple="multiple" id="select2MultipleE">
														...
														@foreach($brand_cities as $city)
															<option value="{{$city->city_id}}" @foreach($brand_message_cities as $brand_message_city) {{ $brand_message_city->city_id == $city->city_id ? 'selected' : '' }} @endforeach>{{$city->city->name}}</option>
														@endforeach
														@endif
													</select>
