...
<script>
  ...
  gtag('config', 'G-Q0VQ8NJD2C');
        ...
        <style type="text/css">
        	...
        	.header {
        		background:<?php echo e($brand_profile->color['header_color'] ?? '#2B004F'); ?>  ;
        	}
        	.footer {
        		background:<?php echo e($brand_profile->color['footer_color'] ?? '#2B004F'); ?>  ;
        	}

        	.loadMore, .commonBtn
        	{
        		background: <?php echo e($brand_profile->color['button_color'] ?? 'linear-gradient(292deg, #EE7047 0%, #F40990 100%)'); ?>  ;
        	}
        	body {
        		color: <?php echo e($brand_profile->color['text_color'] ?? '#26292c'); ?>  ;
        	}
        	 h1, h2, h3, h4, h5, h6 , a{
        	 	color: <?php echo e($brand_profile->color['title_color'] ?? '#26292c'); ?>  ;
        	}
        	.header .navbar-nav>li a {
        		font-family: <?php echo e($brand_profile->font['header_font'] ?? 'PloniMedium'); ?>  ;
        	}

        	.footer-title , .footer .footer-widget p , .inputDiv label , .copyright-text .mb-0 {
        		font-family: <?php echo e($brand_profile->font['footer_font'] ?? 'PloniMedium'); ?>  ;
        	}

        	h1, h2, h3, h4, h5, h6 , a{
        		font-family: <?php echo e($brand_profile->font['title_font'] ?? 'PloniDBold'); ?>  ;
        	}

        	button {
        		font-family: <?php echo e($brand_profile->font['button_font'] ?? 'PloniDBold'); ?>  ;
        	}
        	#contactSection .commonContactDiv .infoDiv .infoDetail {
        		border: 1px solid <?php echo e($brand_profile->color['header_color'] ?? '#2B004F'); ?>

        	}
        	#contactSection .commonContactDiv .infoDiv h5{
        		color:<?php echo e($brand_profile->color['header_color'] ?? '#2B004F'); ?>

        	}
        	#contactSection .commonContactDiv .infoDiv .infoDetail:hover{
			background-color:<?php echo e($brand_profile->color['header_color'] ?? '#2B004F'); ?>
