...
class BrandController extends Controller
{
    ...
    public function store_brand_design(Request $request)
    {
        ...
        // IMAGES
        $files = $brand_profile->website_primary_images ?? [];
        // delete images
        $deleted_images = json_decode($request->deleted_images);
        if ($deleted_images && count($deleted_images) > 0) {
            foreach ($deleted_images as $deleted_image) {
                @unlink(public_path() . "/brand_image/" . $deleted_image);
                $image_key = array_search($deleted_image, $files);
                array_splice($files, $image_key, 1);
            }
        }
        ...
        if ($request->hasfile('images')) {
            foreach (($request->file('images')) as $file) {
                ...
                $file->move('brand_image/', $name);
                array_push($files, $name);
            }
        }
        $brand_profile->website_primary_images = $files;
